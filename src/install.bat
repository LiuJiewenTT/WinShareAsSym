...
:: 主菜单 
reg add "HKCR\Directory\Background\shell\WinShareAsSym" /ve /d "创建链接" /f
reg add "HKCR\Directory\Background\shell\WinShareAsSym" /v "MUIVerb" /d "创建链接" /f
reg add "HKCR\Directory\Background\shell\WinShareAsSym" /v "SubCommands" /d "WinShareAsSymGroup" /f
reg add "HKCR\Directory\Background\shell\WinShareAsSym" /v "Icon" /d "%SystemRoot%\System32\imageres.dll,-5302" /f

:: 定义子菜单组（CommandStore\shell）
set "GROUPBASE=HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\WinShareAsSymGroup"
reg add "%GROUPBASE%" /v "MUIVerb" /d "WinShareAsSymGroup" /f
reg add "%GROUPBASE%" /v "Icon" /d "%SystemRoot%\System32\imageres.dll,-5302" /f
reg add "%GROUPBASE%" /v "Description" /d "WinShareAsSym 子菜单组" /f

:: 子菜单 - 文件符号链接 
reg add "%GROUPBASE%\SymFile" /ve /d "文件符号链接" /f
reg add "%GROUPBASE%\SymFile\command" /ve /d "\"%CURDIR%\creation_wizard\wizard.bat\" \"%%V\" 1" /f

:: 子菜单 - 目录符号链接 
reg add "%GROUPBASE%\SymDir" /ve /d "目录符号链接" /f
reg add "%GROUPBASE%\SymDir\command" /ve /d "\"%CURDIR%\creation_wizard\wizard.bat\" \"%%V\" 2" /f

:: 子菜单 - 文件硬链接 
reg add "%GROUPBASE%\HardLink" /ve /d "文件硬链接" /f
reg add "%GROUPBASE%\HardLink\command" /ve /d "\"%CURDIR%\creation_wizard\wizard.bat\" \"%%V\" 3" /f

:: 子菜单 - 目录联接(Junction) 
reg add "%GROUPBASE%\Junction" /ve /d "目录软联接" /f
reg add "%GROUPBASE%\Junction\command" /ve /d "\"%CURDIR%\creation_wizard\wizard.bat\" \"%%V\" 4" /f

:: 子菜单 - 向导 
reg add "%GROUPBASE%\Wizard" /ve /d "启动生成向导" /f
